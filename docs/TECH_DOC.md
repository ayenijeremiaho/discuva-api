...
**Idle-queue Redis load (`BullModule.forRootAsync` in `app.module.ts`):** Bull runs three per-queue background timers (`drainDelay`, `guardInterval`, `stalledInterval`) that hit Redis on a fixed schedule the moment a queue's `.process()` handler is registered — completely independent of whether any job is ever enqueued. Left at Bull's defaults (5s/5s/30s) across this app's 7 queues, that's on the order of 250k+ idle Redis commands/day, which is what exhausted the production Upstash request quota with zero tenants live (2026-08-12) — not tenant traffic. Fixed by raising all three via the shared root `settings` (`drainDelay: 300`s, `guardInterval: 300000`ms, `stalledInterval: 300000`ms); the `follow-up` queue defines its own `settings` object (needed for its longer `lockDuration`), which fully replaces rather than merges with the root config, so it repeats the same `drainDelay`/`guardInterval` values explicitly. This has no real job-latency cost: Redis's blocking `BRPOPLPUSH` wakes immediately the instant a real job is pushed regardless of the polling ceiling, and `guardInterval`'s ceiling self-adjusts down whenever a delayed job is actually scheduled (none exist anywhere in this codebase currently — no call site uses Bull's `delay`/`repeat` job options). Only real-world trade-off: a worker that crashes mid-job leaves that job "stalled" for up to 5 minutes before Bull reclaims and retries it, instead of 30 seconds — acceptable given the single always-on machine and short-lived handlers this app uses.
