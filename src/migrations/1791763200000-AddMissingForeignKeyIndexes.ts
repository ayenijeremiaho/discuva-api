...
// key (only the referenced/primary-key side gets one) — every FK column
// added across 100+ entities since the original schema was designed had to
// be checked individually, and this backfills every one that was missing
// an index. Without these, both application queries that filter/join on
// these columns and ON DELETE CASCADE/SET NULL cleanup on the referenced
// table do a sequential scan instead of an index scan — invisible on a
// small dev database, increasingly costly as each table grows.
export class AddMissingForeignKeyIndexes1791763200000
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS idx_platform_admins_platform_admin_role_id ON platform_admins(platform_admin_role_id)`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS idx_platform_admins_platform_admin_role_id`,
    );
    await queryRunner.query(
