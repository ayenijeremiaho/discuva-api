...
// Bull processor counterpart to TenantTransactionInterceptor (§4.4): a job
// runs on a worker process with no CLS context of its own, potentially
// long after the request that enqueued it, so tenant context has to be
// re-entered from the envelope carried in job.data rather than inherited.
// Same SET LOCAL-inside-a-transaction mechanism, for the same
// transaction-pooling reason — a plain session-level SET would risk
// leaking across jobs sharing a pooled connection.
//
// Jobs enqueued before any tenant existed (job.data has no schemaName —
// true for every job in the codebase today, since TenantMiddleware isn't
// wired into the live app yet) run exactly as they do now: no transaction
