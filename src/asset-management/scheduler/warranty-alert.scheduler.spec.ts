...
import { WarrantyAlertScheduler } from './warranty-alert.scheduler';
...
describe('WarrantyAlertScheduler', () => {
  let scheduler: WarrantyAlertScheduler;
  ...
  it('sends an alert and stamps the notified date when warranty expires in 30 days', async () => {
    mockTenantRepo.find.mockResolvedValue([
      { id: 't1', subdomain: 'a', schemaName: 'church_a' },
    ]);
  ...
  it('continues past one tenant failing so the rest still get processed', async () => {
    ...
    await expect(scheduler.dispatchWarrantyAlerts()).resolves.toBeUndefined();
